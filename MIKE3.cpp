...
int compare(int x,int y){
	...
	i=j=0;
	while(i!=xlen && j!=ylen){
		if(stamps[x][i]==stamps[y][j])
			return 0;
		else if(stamps[x][i]>stamps[y][j])
			++j;
		else
			++i;
	}
...
int main()
{
 ...
 int n, i, j, k, K,M, p, q, r, s, min, edge, counter=0;
 //infile>>n;
 n=readint();
 n=M=readint();
 for(int i=0;i<M;++i){
...
/*
 ...
 K=n;
  //cout<<"Find an Independent Set of size at least k = " <<K;
// cin>>K;
 k=n-K;
